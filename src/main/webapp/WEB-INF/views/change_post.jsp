...
<c:set var="now" value="${post.publishedAt}" />	
<c:set var="user" value="${post.user}" />	
 
	<div id="container">
	
		
		<form:form role="form" action="${pageContext.request.contextPath}/posts/changed/post${postAndCategoriesWrapper.post.id}" 
			modelAttribute="postAndCategoriesWrapper" id="postForm">
		<table class="table">
			<tbody>
			<tr> 
			 	<th class="form-group col-md-8">
					<form:input type="hidden" class="form-control" path="post.id" id="postId" />	
				</th>
				<th></th>
			</tr>
			<tr> 
			 	<th class="form-group col-md-8">
					...
					<form:input class="form-control" path="post.title" id="postTitle" />
			...
			<tr>	
				<th class="form-group">
					<label for="publishedOptions">Opublikowany? </label>
